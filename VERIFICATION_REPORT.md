...
| **P.618 강우감쇠 10-step (직접 제작)** | ⚠ Step 2~9 + Step 10 구조 모두 정확하나, **Step 10 β 3번째 분기 누락 (실결과 영향)** + θ<5° 식 누락(잠재) |
...
| **위성툴박스 사용 (AI 제작)** | ✅ 전 함수 단위·인수순서·G/T 처리·이중계상 없음 실측 확인. 단 **gs0/gsO 오타로 실행 불가** |
| **인터페이스/배선 (AI 제작)** | ✅ 단위 체인·구조체 계약·벡터 방향 전부 안전. 단 Phase-4 self-check가 부분실행 토글과 충돌 |

**핵심 메시지**: 직접 제작한 ITU 수식들은 매우 충실하게 구현됨 (β 분기 1건 제외). AI 제작 부분도 툴박스 사용은 정확하나 실행을 막는 오타 1건과 토글 버그 1건 존재. 가장 큰 **논문 차원의** 이슈는 코딩이 아니라 **모델 정합성**: ① P.618 §2.5의 freeze 규칙 위반(구름·가스를 0.1% 조건으로 입력 → E/W밴드에 불리한 비대칭 비관 편향), ② 73.5 GHz가 P.618 유효범위(≤55 GHz) 밖이라는 외삽 명시 필요.
...
**[F-1] `gs0`(숫자 0) ↔ `gsO`(대문자 O) 미정의 변수 — main_v2.m:139–140**
...
**[F-2] P.618 Step 10 β 3번째 분기 누락 — CalcRainLoss.m:121–126**
...
**[F-3] P.618 §2.5 freeze 규칙 위반: 구름 A_C를 0.1% 값으로 사용 — LinkBudget.m:56 + GroundStations.m**
...
**[F-4] 가스 A_G도 0.1% 조건의 ρ_surf로 계산 — LinkBudget.m:50 + GroundStations.m**
- 검증 과정에서 로컬 P.836 맵([WP_2]\ITURDigitalMaps\p836.mat)을 보간해 GroundStations의 rho_surf가 실제로 **P.836 0.1% 초과확률 값**임을 확인(예: Stuttgart 16.04 vs 맵 16.30; 중앙값은 6.95). 헤더 문서 그대로.
...
**[F-5] Phase-4 self-check가 부분실행 토글을 깨뜨림 — main_v2.m:250–262**
- run_energy=false → 252행 assert 즉사; run_volume=false → 259행 비존재 필드 참조로 즉사 (R2025b 라이브 재현 완료). 클로저 보정 루프(234행)만 가드되어 있음 — 명백한 누락.
- 수정: self-check (1)을 `if cfg.run_energy`, (2)를 `if cfg.run_volume`로 감싸기.

**[F-11] N_wet도 중앙값이 아닌 고초과확률 값 사용 — GroundStations.m (2026-06-10 신규 확인)**
...
- 로컬 ITURDigitalMaps(p453.NWET_Annual_50, P.453 08/2019) 보간 결과 중앙값은 45.4/47.5/132.0/88.1/19.3 (Stuttgart/KAU/Singapore/UAE/SvalSat)인데, 하드코딩값은 101.3/143.1/156.4/172.7/48.7 — **중앙값의 2~3.5배** (0.1%급 추출로 추정).
...
**[F-10] CalcRainLoss.m:74–76 — 함수 내부 디버그 figure 15개/런 생성** (cfg.show_plots 무시, plot 제목의 전제 "γ_R은 앙각 무관"도 τ=45°에서만 참)
...
- 데드 코드/필드: `cfg.req_EbNo_dB`(미사용 — band.reqEbNo_dB가 실사용), `cfg.show_plots`(읽는 곳 없음), `gs.id`/`pass_start`/`pass_stop`/`band.name`, DailyVolume의 `gs` 인수, CalcRainLoss 71–73행 transpose 데드코드, "rainpl 사용" 낡은 주석
...
- main_v2.m:73 'Groud Stations()' 오타, 193행 주석 필드명 불일치
- 신틸레이션/강우/구름의 자체 앙각 하한(4°/0.1°/0.1°)이 main 마스크 5°에 기생 — LinkBudget에 `assert(min(el_vis)>=5)` 권장

## 6. 실행·디버깅 계획 (보고용 — 아직 미실행)

**전제**: F-1(gs0) 수정 없이는 어떤 실행도 불가. F-2~F-5 수정 여부에 따라 결과가 달라지므로 "수정 전 1회 실행(베이스라인) → 수정 후 재실행(차이 정량화)" 순서가 논문 기록용으로 유리.
...
| # | 항목 | 파일:행 | 심각도 | 상태 |
|---|---|---|---|---|
| 1 | `gs0`→`gsO` | main_v2.m:139–140 | **P0 실행차단** | ✅ **적용 완료** (2026-06-11, 각주 포함; 코드 내 gs0 잔존 0건 확인) |
| 2 | β 3번째 분기(θ<25°) 추가+벡터화 | CalcRainLoss.m:121–126 | **P1 결과오류** | ✅ **적용 완료** (2026-06-11, 이론비 4자리 일치 검증) |
...
| 5 | self-check 토글 가드 | main_v2.m:250–262 | **P1 토글버그** | 미수정 |
| 6 | `./(2.*k)` | CalcRainCoeffs_Exact.m:81 | P2 잠재 | 미수정 |
| 7 | θ<5° Ls Eq.(2) | CalcRainLoss.m:44 | P2 잠재 | 📝 각주만 추가(설계 결정: GS 최소앙각 5° — 2026-06-11) |
| 8 | 디버그 figure 제거 | CalcRainLoss.m:74–76 | P2 위생 | ✅ **적용 완료** (2026-06-11, 주석 보존+각주; figure 0개·수치 불변 검증) |
...
→ 코드(CalcRainLoss.m:44)는 el>0.1° 전 구간에 Eq.(1)만 적용 — Eq.(2) 부재.
...
→ 코드(CalcRainLoss.m:122–126)는 첫 두 분기만 구현, 두 번째 분기의 "θ ≥ 25°" 조건 자체가 없어 θ<25°에도 두 번째 식을 적용. "Otherwise" 분기 부재.
...
**소스 및 방법**: `[WP_2]\ITU_Doc\`의 ITU 공식 디지털 product TXT (P.840-9 Part01 / P.836-6 annual / P.453-14 NWET annual / P.837-8(2025-09) / P.839-4) + 사용자 자체 추출 스크립트(`find_L_value.m`, `GetSurfaceVaporDensity.m`, `find_N_wet_value.m`, `get_R001_value.m`, `get_h0_value.m`)와 동일한 bilinear 보간 규약.
...
| **rho_surf [g/m³] ← RHO(5%)** (Eq.68) | **12.3758** | **20.0830** | **22.2136** | **24.7600** | **5.4433** | 교체 |
...
주: ① rho는 WP2 방식 그대로 순수 bilinear(고도보정 없음) — §9 구판(MathWorks maps.mat 기반, 고도보정 포함) 수치와 ±4% 이내 차이는 방법 차이이며, 사용자 파이프라인과의 일관성을 위해 본 표의 공식 TXT 값을 채택. ② 로컬 p840.mat(MathWorks)은 P.840-8 L_red라 정의가 달라 사용 금지 — 본 표의 L은 P.840-9 공식 product 직접 추출값.
