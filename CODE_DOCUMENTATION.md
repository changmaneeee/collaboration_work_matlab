...
> 대상: `main_v2.m` 및 지원 함수 12개 (MATLAB R2025b; 2026-06-11 `ApplyAvailabilityInputs`·`ExtractITUPercentileInputs` 추가)
...
Phase 1  입력층        BandParameters() / GroundStations() 로드, 선택 검증
                        → ApplyAvailabilityInputs(): cfg.avail에 맞는 ITU 입력(L/ρ/N_wet) 적용
                          + 강우항 포함여부 태깅(p>5%면 제외) + 적용 테이블 출력  [2026-06-11]
...
| `avail` | 0.999 | 링크 가용도 → ITU 초과확률 p = 0.1%. **유효값은 availability_input_candidates.csv 수록 11종(0.70/0.80/0.90/0.95/0.98/0.99/0.995/0.998/0.999/0.9995/0.9999)** — Phase 1의 ApplyAvailabilityInputs가 검증·적용, 미수록 값은 에러 |
...
**`GroundStations.m`** — 지상국 5개소 struct-of-structs. 각 국소별 ITU-R 파라미터는 **P.618-14가 요구하는 통계량별 조건**으로 디지털 맵에서 추출된 값을 하드코딩 (2026-06-11 P.618-14 준수 세트로 갱신, 구 0.1% 일괄 추출값은 인라인 주석으로 보존):
`rho_surf`(P.836, g/m³, **5% 초과확률·Eq.(68) freeze·공식 고도보정 포함**) / `h_rain_limit_m`(P.839, m, 연평균 h0+0.36 km) / `R_001`(P.837, mm/h, **0.01%** 초과확률) / `L_water`(P.840, kg/m², **5% 초과확률·Eq.(67) freeze**) / `N_wet`(P.453, N-unit, **연 중앙값 50%**).
하드코딩값은 avail=0.999 기본값에 해당하며, 실행 시 ApplyAvailabilityInputs가 cfg.avail에 맞는 CSV 값으로 덮어씀.
...
- **[2026-06-11] 강우항 스위치**: `gs.rain_included`(ApplyAvailabilityInputs가 태깅)가 false면 — 즉 p > 5%(가용도 ≤ 90%)면 — P.618-14 §2.5 Eq.(66)에 따라 강우항을 0으로 제외. 태그 없는 레거시 호출은 기존 동작(항상 포함) 유지.
...
### 3.4 가용도 입력 계층 (2026-06-11 추가)

**`ApplyAvailabilityInputs.m`** — cfg.avail에 맞는 가용도별 ITU 입력 적용기.
- `availability_input_candidates.csv`(가용도 11종 × 5국 = 55행)에서 해당 행을 읽어, 선택된 지상국('all'/부분/단일 동일 동작)의 `L_water/rho_surf/N_wet`을 교체하고 적용 테이블을 콘솔 출력
- `rain_included`(p>5%면 false → LinkBudget이 강우항 제외, Eq.(66))·`combination`·`scint_a_p`·`p_pct` 태깅
- 미수록 가용도는 유효 목록과 함께 에러; `R_001`/`h_rain_limit_m`은 정의상 가용도 불변이라 교체하지 않고 CSV와 교차검증만 수행

**`ExtractITUPercentileInputs.m`** — 루트 R-REC-* 공식 디지털 맵에서 임의 백분위의 L(P.840-9, bilinear) / ρ(P.836-6 **공식 고도보정**: 4노드 VSCH 지수 스케일링, 노드고도는 TOPO_0DOT5 **bicubic** 보간) / N_wet(P.453-14, bilinear)을 재추출하는 도구. 기존 CSV 앵커값(5%/10%)을 기계 정밀도(<5e-14), heritage 0.1% 값을 4자리(<4.5e-05)로 재현함을 검증 완료 — 새 지상국·새 백분위 추가 시 이 도구 사용.
...
- **P0 (실행차단)**: ~~main_v2.m `gs0`(숫자0) ↔ `gsO`(대문자O) 오타~~ → ✅ **수정 완료** (2026-06-11, 현 main_v2.m:151–159 FIX 주석)
- **P1 (결과오류)**: ~~CalcRainLoss β 3번째 분기(θ<25°) 누락~~ → ✅ 수정 완료(2026-06-11, 3분기 전부 구현) / ~~구름·가스 입력의 P.618 §2.5 freeze 규칙 위반~~ → ✅ 수정 완료(GroundStations P.618-14 준수 세트) / Phase-4 self-check가 부분실행 토글과 충돌 (**미수정** — `run_link/energy/volume` 전부 true 유지할 것)
- **P2 (잠재)**: 벡터입력 시 `/`·`^` 연산자 버그(현재 미발현), θ<5° 강우식(Eq.(2))은 **의도적 미구현으로 확정**(5° 앙각 마스크 전제 — CalcRainLoss Step 2 각주), ~~디버그 figure 15개/런~~ → ✅ 비활성화 완료(2026-06-11), 신틸레이션 L 간략식
